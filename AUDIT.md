...
| M5 Advisory CI scans | **Fixed** | OWASP job now enforcing (fails on CVSS ≥ 7); Trivy fails on CRITICAL (wired to a `.trivyignore` for documented false positives/accepted risks, mirroring `dependency-check-suppressions.xml`); CodeQL stays artifact-only (upload deliberately removed earlier — code scanning not enabled on the repo) |
